...
FileServiceContract\CodeGen_Client.config      Alternate config file (see note at end)
FileServiceContract\CodeGen_Host.config        Alternate config file (see note at end)
...
building the FileServiceContract project.
...
To run the service type:
  ...
  FileService.exe <path>  (where <path> is the folder of files to be served)

To run the client type:

  FileClient.exe <path> <device ID> (where <path> is the folder to receive files
                                     and <device ID> is the ID printed by
                                     FileService.exe)
...
1)  Modify the FileServiceContract pre-build step to use
    CodeGen_Client.config or CodeGen_Host.config instead of CodeGen_All.config
