...
const WCHAR OBJECT_NAME[] = L"3D-Printer Service Proxy";
...
{
	HRESULT hr = E_FAIL;
	CPrinterProviderFactory* pCPrinterProviderFactory = NULL;

	if (rclsid != CLSID_SsysPrinterSvcProxy)
	{
		return CLASS_E_CLASSNOTAVAILABLE;
	}

	pCPrinterProviderFactory = new CPrinterProviderFactory();
	if (NULL == pCPrinterProviderFactory)
	{
		return E_OUTOFMEMORY;
	}

	hr = pCPrinterProviderFactory->QueryInterface(riid, ppv);
	pCPrinterProviderFactory->Release();
